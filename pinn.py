...
import numpy as np
from n_laaf import NLLAFLayer
from laaf import LLAFLayer
import psutil
...
class PINN(tf.keras.Model):
    def __init__(self, 
                 ...
                 t_b, x_b, u_b, 
                 t_r, x_r, 
                 Rm, Rn, Rl, depth, af_method, activ = "sigmoid", BN = False,
                 w_init = "glorot_normal", b_init = "zeros", 
                 lr = 1e-3, opt = "Adam", w_0 = 0.9, w_b = 0.9, w_r = 1.2,
        ...
        self.af_method = af_method #what type of adaptive AF is used: 0 = no adaptive AF, 1 = LLAF, 2 = NLLAF
    ...
    def dnn_init(self, Rm, Rn, Rl, depth):
        ...
        Rm = (Rm,) if isinstance(Rm, int) else Rm
        ...
        if self.BN == True:   # False by default
            ...
            elif self.af_method == 1:
                for l in range(depth - 1):
                    network.add(LLAFLayer(Rl, activation = self.activ, use_bias = True,
                                                      kernel_initializer = self.w_init, bias_initializer = self.b_init,
                                                      kernel_regularizer = None, bias_regularizer = None,
                                                      activity_regularizer = None, kernel_constraint = None, bias_constraint = None))

            elif self.af_method == 2:
                for l in range(depth - 1):
                    network.add(NLLAFLayer(Rl, activation=self.activ, use_bias=True,
                                          kernel_initializer=self.w_init, bias_initializer=self.b_init,
                                          kernel_regularizer=None, bias_regularizer=None,
                                          activity_regularizer=None, kernel_constraint=None, bias_constraint=None))
    ...
    def get_final_ram_usage(self):
        """Returns the final RAM usage of the current process."""
        process = psutil.Process()
        memory_info = process.memory_info()
        ram_usage = memory_info.rss / (1024 ** 2)  # Convert bytes to MB
        return ram_usage
    def train(self, start_time, thresholds, tol, epoch = 10 ** 5, batch = 2 ** 6):
        start = time.time()
        k = 0
        print(">>>>> training setting;")
        if self.af_method == 0:
            print("         af_method      : no adaptive AF")
        elif self.af_method == 1:
            print("         af_method      : layer-wise locally adaptive AF")
        elif self.af_method == 2:
            print("         af_method      : neuron-wise locally adaptive AF")
        print("         # of epoch     :", epoch)
        print("         batch size     :", batch)
        print("         convergence tol:", tol)
        os.makedirs(f"figures/no_af", exist_ok=True)
        os.makedirs(f"figures/laaf", exist_ok=True)
        os.makedirs(f"figures/n_laaf", exist_ok=True)
        file_name_loss = ""
        file_name_thresholds = ""
        if self.af_method == 0:
            dir_name = f"figures/no_af/{start_time}_{self.activ}"
            os.makedirs(dir_name, exist_ok=True)
            file_name_loss = f"figures/no_af/{start_time}_{self.activ}/log.txt"
            file_name_thresholds = f"figures/no_af/{start_time}_{self.activ}/thresholds.txt"
        elif self.af_method == 1:
            os.makedirs(f"figures/laaf/{start_time}", exist_ok=True)
            file_name_loss = f"figures/laaf/{start_time}/log.txt"
            file_name_thresholds = f"figures/laaf/{start_time}/thresholds.txt"
        elif self.af_method == 2:
            os.makedirs(f"figures/n_laaf/{start_time}", exist_ok=True)
            file_name_loss = f"figures/n_laaf/{start_time}/log.txt"
            file_name_thresholds = f"figures/n_laaf/{start_time}/thresholds.txt"
        t0 = time.time()
        ...
        for ep in range(epoch):
            ...
            if ep_loss < thresholds[k]:
                k = k + 1
                with open(file_name_thresholds, "a") as file:
                    file.write(f"error {thresholds[k]} at epoch {ep}\n")
            if ep % self.f_mntr == 0:
                elps = time.time() - t0
                self.ep_log.append(ep)
                self.loss_log.append(ep_loss)
                log = "ep: %d, loss: %.3e, elps: %.3f" % (ep, ep_loss, elps)
                with open(file_name_loss, "a") as file:
                    file.write(log)
                    file.write('\n')
                print(log)
                t0 = time.time()
        ...
        with open(file_name_loss, "a") as file:
            ...
            if (time_elapsed < 60):
                file.write(f"Execution time: :{time_elapsed:.3f} seconds")
            elif (time_elapsed < 3600):
                file.write(f"Execution time: :{time_elapsed // 60:.0f} minutes and {time_elapsed % 60:.3f} seconds")
            elif (time_elapsed < 216000):
                file.write(f"Execution time: :{time_elapsed // 3600:.0f} hours, {(time_elapsed % 3600) // 60:.0f} minutes and {(time_elapsed % 3600) % 60:.3f} seconds")
        # final_ram_usage = self.get_final_ram_usage()
        # print(f"Final RAM Usage: {final_ram_usage:.2f} MB")
        print(">>>>> end time:", datetime.now())
